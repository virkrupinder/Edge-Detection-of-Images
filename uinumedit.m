...
case 'Increase'
   hEditBox = findobj('Type','uicontrol','Tag',varargin{2});
   StepSize = varargin{3};
   Max = varargin{4};
   CallBack = varargin{5};
...
otherwise
   ...
   CallBacks = char({ ...
         sprintf('uinumedit(''Increase'',''%s'',%.16f,%.16f,''%s'')', ...
         Tag,StepSize,Max,strrep(CallBack,'''','''''')); ...
         sprintf('uinumedit(''Decrease'',''%s'',%.16f,%.16f,''%s'')', ...
         Tag,StepSize,Min,strrep(CallBack,'''','''''')); });
